...
# Load inputs from input_1.json
# 1. Get session key using email and password - use this for subsequent requests
# 2. Create profile, get profile id
# 3. Start test, get test id
# 4. Poll test status until error or completed, get report id
# 5. Get test report
...
INPUTS_FILE = Path(__file__).with_name("input_1.json")
...
def create_profile(api_key, session_key, profile_info):
    ...
    payload = {
        ...
        "endpoint_config_id": profile_info["endpoint_config_id"],
        "config": json.dumps(profile_info["config"]),
        "retry_attempts": profile_info["retry_attempts"],
    }
...
def main():
    try:
        ...
        profile_id = create_profile(api_key, session_key, inputs["profile_info"])
        print("Profile created successfully! Profile ID: ", profile_id)
        test_id = start_test(
            api_key, session_key, profile_id, inputs["report_info"]["name"]
        )
